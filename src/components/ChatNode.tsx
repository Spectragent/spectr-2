...
export function ChatNode({ id, data: initialData }: NodeProps) {
  ...
  const updateMessageWithTokenMetrics = async (
    inputTok: number,
    ...
    const selectedModel = [...availableModels, ...(settings.customModels || [])].find(
      m => m.id === model
    );
  ...
  const getEndpointForModel = (selectedModel: AIModel) => {
    ...
    return selectedModel.provider === 'openai'
      ...
      : selectedModel.provider === 'deepseek'
        ...
        : selectedModel.provider === 'perplexity'
          ? 'https://api.perplexity.ai'
          : selectedModel.provider === 'xai'
            ? 'https://api.x.ai/v1'
            : selectedModel.provider === 'wai'
              ? 'https://api.w.ai/v1'
              : selectedModel.provider === 'nvidia'
                ? '/api/nvidia'
                : selectedModel.provider === 'inferencenet'
                  ? 'https://api.inference.net/v1'
                  : selectedModel.provider === 'alibabacloud'
  ...
  const getModelIdentifier = (selectedModel: AIModel) => {
    if ('endpoint' in selectedModel) {
      // For local models (like Ollama), use the name instead of id
      return (selectedModel as CustomModel).name;
    }
    // For remote models, use the id
  ...
  const getApiKeyForModel = (selectedModel: AIModel | CustomModel) => {
    ...
    return selectedModel.provider === 'xai'
      ? settings.xai?.apiKey || ''
      : selectedModel.provider === 'perplexity'
        ...
        : selectedModel.provider === 'wai'
          ...
          : selectedModel.provider === 'nvidia'
            ...
            : selectedModel.provider === 'inferencenet'
              ? settings.inferencenet?.apiKey || ''
              : selectedModel.provider === 'alibabacloud'
                ? settings.alibabacloud?.apiKey || ''
                : selectedModel.provider === 'deepseek'
      ...
      if (selectedModel.provider === 'anthropic') {
        ...
        if (settings.streaming) {
          ...
          const assistantMessage: Message = {
            ...
            metrics: {
              tokensPerSecond: metrics.completion_tokens
                ? metrics.completion_tokens / totalTime
                : undefined,
        ...
        const isO1Model =
          model.includes('o1-mini') || model.includes('o1') || model.includes('o1-preview');
        const messagesToSend = sanitizeChatMessages([
          ...(!isO1Model && enhancedSystemPrompt
  ...
  const currentModel = [...availableModels, ...(settings.customModels || [])].find(
    m => m.id === model
  );
  ...
  return (
    <>
      ...
      />
      <Card className="w-[650px] bg-card" style={{ minWidth: '100%', width: '650px' }}>
          ...
          <div className="space-y-4">
            {messages.map((msg, i) => (
              ...
              >
                ...
                {msg.image_url && (
                  <img src={msg.image_url} alt="Uploaded" className="max-w-sm rounded-md" />
                )}
                {msg.content &&
                  (() => {
                            ...
                            <ThinkingBlock key={block.key}>
                              <ReactMarkdown
                                components={{
                                  ...
                                  pre: ({ children }) => (
                                    <pre className="p-0 m-0" onClick={e => e.stopPropagation()}>
                                      {children}
                                    </pre>
                                  ),
                                  h1: ({ children }) => (
                    ...
                    <img
                      src={
                        currentModel && 'endpoint' in currentModel
                          ? PRESET_ENDPOINTS.find(p => p.url === currentModel.endpoint)?.icon ||
                  ...
                  <SelectItem key={m.id} value={m.id}>
                    <div className="flex items-center gap-2">
                      <img
                        src={
                          'endpoint' in m
                            ? PRESET_ENDPOINTS.find(p => p.url === m.endpoint)?.icon ||
                              m.thumbnailUrl ||
                              logo
                            : m.thumbnailUrl || logo
                        }
